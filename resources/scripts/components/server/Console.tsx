...
export default () => {
    ...
    const TERMINAL_PRELUDE = '\u001b[1m\u001b[33m'+name+'@iGame~ \u001b[0m';
    ...
    const handleTransferStatus = (status: string) => {
        switch (status) {
            case 'failure':
                terminal.writeln(TERMINAL_PRELUDE + 'Le transfert à échouer...\u001b[0m');
                return;

            case 'archive':
                terminal.writeln(TERMINAL_PRELUDE + 'Le serveur a été archivé avec succès, tentative de connexion au serveur...\u001b[0m');
        }
    };
    ...
    const handlePowerChangeEvent = (state: string) => terminal.writeln(
        TERMINAL_PRELUDE + 'Le service est actuellement ' + state + '...\u001b[0m',
    );
        ...
        <div css={tw`text-xs font-mono relative`}>
            ...
            <div css={tw`rounded-b bg-neutral-900 text-neutral-100 flex items-baseline`}>
                ...
                <div css={tw`w-full`}>
                    <CommandInput
                        type={'text'}
                        placeholder={'Veuillez écrire une commande...'}
                        aria-label={'Commande.'}
